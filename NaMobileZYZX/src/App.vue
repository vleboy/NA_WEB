        ...
        axios.get(`${url}/webapi/ipquery`)
          .then(function (res) {
            if(res.data.payload.data.country === '中国'){
              window.location.href = `http://app.risheng3d.com/apk/ZYZX${res.data.payload.data.zyzxversion}.apk`
            } else {
              window.location.href = `http://oss.na12345.com/ZYZX${res.data.payload.data.zyzxversion}.apk`
            }
          })
         ...
         .then(function (res) {
           if(res.data.payload.data.country === '中国'){
             window.location.href = `itms-services://?action=download-manifest&url=https://assetdownload.oss-cn-hangzhou.aliyuncs.com/ios/ZYZXlist${res.data.payload.data.zyzxversion}.plist`
           } else {
             window.location.href = `itms-services://?action=download-manifest&url=https://oss.na12345.com/ZYZXlist${res.data.payload.data.zyzxversion}.plist`
           }
         })
         .catch(function (error) {
           console.log(error)
