  ...
  methods: {
    ...
    async beginGame (gameType, sid, ttgGameType, ttgGameId, ttgGameName) {
      try {
        ...
        if (data.code === 0) {
          window.location.href = `http://ext.na77.org.s3-website-ap-southeast-1.amazonaws.com/mg_detail.html?parent=${param.parent}&userId=${param.userId}&userName=${param.userName}&token=${param.token}&gameurl=${encodeURIComponent(data.url)}`
          this.loading = false
        } else {
          this.loading = false
          alert(data.msg)
