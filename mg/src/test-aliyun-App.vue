  ...
  methods: {
    ...
    async beginGame (gameType, sid, ttgGameType, ttgGameId, ttgGameName) {
      try {
        ...
        if (data.code === 0) {
          window.location.href = `http://web.risheng3d.com/mg_detail.html?parent=${param.parent}&userId=${param.userId}&userName=${param.userName}&gameId=${param.sid}&gameurl=${encodeURIComponent(data.url)}&token=${param.token}`
          this.loading = false
        } else {
          this.loading = false
          alert(data.msg)
