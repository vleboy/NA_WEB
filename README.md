# NA_WEB

## SB测试服
http://web.risheng3d.com/sb_video.html?fullscreen=false&userId={userId}&token={token}
http://ext.na77.org.s3-website-ap-southeast-1.amazonaws.com/sb_video.html?fullscreen=false&userId={userId}&token={token}
http://web.risheng3d.com/sb_live.html?fullscreen=false&userId={userId}&token={token}
http://ext.na77.org.s3-website-ap-southeast-1.amazonaws.com/sb_live.html?fullscreen=false&userId={userId}&token={token}
## SB正式服
http://webpage.risheng3d.com/sb_video.html?fullscreen=false&userId={userId}&token={token}
http://webpage.na77.org/sb_video.html?fullscreen=false&userId={userId}&token={token}
http://webpage.risheng3d.com/sb_live.html?fullscreen=false&userId={userId}&token={token}
http://webpage.na77.org/sb_live.html?fullscreen=false&userId={userId}&token={token}